...
router.post('/register', (req, res) => {
  ...
  if (!email || !password || !name || !role) return res.status(400).json({ error: 'Thiếu thông tin bắt buộc' })
...
export function authenticate(req, res, next) {
  ...
  if (!auth?.startsWith('Bearer ')) return res.status(401).json({ error: 'Không có token xác thực' })
  try {
...
export function requireRole(role) {
  return (req, res, next) => {
    if (req.user?.role !== role) return res.status(403).json({ error: `Chỉ ${role} mới có quyền truy cập` })
    next()
  }
}
