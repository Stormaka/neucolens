...
interface User { id: number; email: string; name: string; role: 'teacher' | 'student'; studentCode?: string }
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    auth.me().then((u: User) => setUser(u)).catch(() => {
  ...
  const logout = () => {
    auth.logout(localStorage.getItem('refresh_token')).catch(() => undefined)
    localStorage.removeItem('access_token')
    localStorage.removeItem('refresh_token')
    setUser(null)
  }
